...
def get_db_connection():
    ...
    return psycopg2.connect(
        host=DB_HOST,
        database=os.getenv("DB_NAME", "meydan_db"),
...
def ziyaretcileri_getir():
    ...
    if cached_data:
        return {"kaynak": "Redis (Çalışma Masası'ndan geldi 🚀)", "veriler": json.loads(cached_data)}
    ...
    cache.set("ziyaretci_listesi", json.dumps(ziyaretciler), ex=20)
    
    return {"kaynak": "PostgreSQL (Arşiv Odası'ndan geldi 🐢)", "veriler": ziyaretciler}